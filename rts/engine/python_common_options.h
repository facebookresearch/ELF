...
struct PythonOptions {
    ...
    PythonOptions()
      : simulation_type(ST_NORMAL), map_size_x(20), map_size_y(20), max_unit_cmd(10), max_tick(30000), seed(0), shuffle_player(false), mcts_threads(1), mcts_rollout_per_thread(1), mcts_verbose(false), game_name(0), handicap_level(0) {
    }
    ...
    void Print() const {
        ...
        std::cout << "MCTS #threads: " << mcts_threads << " #rollout/thread: " << mcts_rollout_per_thread << ", verbose = " << (mcts_verbose ? "True" : "False") << std::endl;
    ...
    REGISTER_PYBIND_FIELDS(simulation_type, map_size_x, map_size_y, max_unit_cmd, output_filename, cmd_dumper_prefix, map_filename, save_replay_prefix, max_tick, seed, mcts_threads, mcts_rollout_per_thread, game_name, handicap_level, shuffle_player, mcts_verbose);
};
